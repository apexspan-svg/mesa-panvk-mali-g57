                                ...
                                VkQueryPool *pQueryPool)
{
   ...
   case VK_QUERY_TYPE_PIPELINE_STATISTICS: {
      /* CPU-backed stub: the Mali-G57 does not expose the GPU counters
       * needed for true pipeline-statistics.  We still accept the pool so
       * DXVK can create it (it does so at device init and crashes if the
       * creation fails); queries are marked available immediately on the
       * host and read back as zeros. */
      reports_per_query = MAX2(1u, util_bitcount(pCreateInfo->pipelineStatistics));
      pool->is_pipeline_stats_stub = true;
      break;
   }
   ...
   default:
      ...
      return vk_errorf(device, VK_ERROR_FEATURE_NOT_PRESENT,
                       "Unsupported query type: %u", pCreateInfo->queryType);
   }
