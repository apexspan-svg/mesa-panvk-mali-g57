...
panvk_per_arch(cmd_close_batch)(struct panvk_cmd_buffer *cmdbuf)
{
   ...
   /* The v9 rendering path allocates an FBD at EndRendering even without
    * draws.  Its attachment loads/stores still require a fragment job. */
   if (batch->fb.desc.cpu
#if PAN_ARCH < 9
       && (batch->vtc_jc.first_tiler ||
           cmdbuf->state.gfx.render.fb.needs_store || getenv("PANVK_FORCE_FRAG"))
#endif
   ) {
