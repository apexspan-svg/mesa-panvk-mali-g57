   ...
   struct vk_device_extension_table *ext)
{
   ...
   *ext = (struct vk_device_extension_table){
      ...
      /* kbase uses the SAME_VA memory model and cannot map a BO at a
       * caller-chosen host address, which VK_EXT_map_memory_placed requires.
       */
      .EXT_map_memory_placed = device->kbase_node_path[0] == '\0',
   ...
   const struct panvk_physical_device *device, struct vk_features *features)
{
   ...
   *features = (struct vk_features){
      ...
      /* ADHOC-DXVK-FL: DXVK 1.10.3 requires geometry/tessellation features
       * for D3D_FEATURE_LEVEL >= 10_0.  Temporarily advertise them to unblock
       * the D3D11 probe.  Revert if pipeline creation asserts with GS/TCS/TES. */
      .geometryShader = true,
      ...
      .textureCompressionBC = /* ADHOC-DXVK-FL override */ true,
      .occlusionQueryPrecise = true,
      /* Pipeline statistics queries are not implemented. */
      ...
      /* ADHOC-DXVK-FL: DXVK 10_0+ requires transformFeedback + geometryStreams */
       ...
       *
       * kbase/SAME_VA cannot honor caller-chosen placed mappings.
       */
      .memoryMapPlaced = device->kbase_node_path[0] == '\0',
      .memoryMapRangePlaced = false,
      .memoryUnmapReserve = device->kbase_node_path[0] == '\0',
