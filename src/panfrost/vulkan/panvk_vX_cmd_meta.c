   ...
   const VkCopyImageToBufferInfo2 *pCopyImageToBufferInfo)
{
   ...
   VK_FROM_HANDLE(panvk_buffer, dst, pCopyImageToBufferInfo->dstBuffer);
   struct vk_meta_copy_image_properties img_props =
      panvk_meta_copy_get_image_properties(img, false, false);

   if (getenv("PANVK_TRACE_WSI_COPY") && pCopyImageToBufferInfo->regionCount) {
   ...
   if (getenv("PANVK_PROBE_USERBUF_FILL") &&
       pCopyImageToBufferInfo->regionCount) {
      const VkBufferImageCopy2 *r = &pCopyImageToBufferInfo->pRegions[0];
      uint64_t row_length = r->bufferRowLength ?
                            r->bufferRowLength : r->imageExtent.width;
      uint64_t fill_size = row_length *
   ...
   if (getenv("PANVK_COPY_PROBE_RAW") &&
       pCopyImageToBufferInfo->regionCount == 1 && img->plane_count == 1 &&
       img->vk.drm_format_mod == DRM_FORMAT_MOD_LINEAR) {
      const VkBufferImageCopy2 *r = &pCopyImageToBufferInfo->pRegions[0];
      const struct pan_image_slice_layout *slice = &img->planes[0].plane.layout.slices[0];
      const uint64_t pitch = (uint64_t)r->imageExtent.width * 4;
      const uint64_t size = pitch * r->imageExtent.height;
      if (vk_format_get_blocksize(img->vk.format) == 4 &&
          r->imageSubresource.aspectMask == VK_IMAGE_ASPECT_COLOR_BIT &&
          r->imageSubresource.mipLevel == 0 &&
          r->imageSubresource.baseArrayLayer == 0 &&
          r->imageSubresource.layerCount == 1 && r->imageExtent.depth == 1 &&
          r->imageOffset.x == 0 && r->imageOffset.y == 0 && r->imageOffset.z == 0 &&
          (!r->bufferRowLength || r->bufferRowLength == r->imageExtent.width) &&
          slice->tiled_or_linear.row_stride_B == pitch && size <= slice->size_B &&
          r->bufferOffset <= dst->vk.size && size <= dst->vk.size - r->bufferOffset) {
         /* Diagnostic alias: vk_meta_copy_buffer records only the addresses
          * in push constants. The source image retains the actual allocation. */
         struct panvk_buffer alias;
         const VkBufferCreateInfo alias_info = {
            .sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
            .size = size,
            .usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT |
                     VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
         };
         vk_buffer_init(&dev->vk, &alias.vk, &alias_info);
         alias.vk.device_address = img->planes[0].plane.base + slice->offset_B;
         const VkBufferCopy2 region = {
            .sType = VK_STRUCTURE_TYPE_BUFFER_COPY_2,
            .dstOffset = r->bufferOffset,
            .size = size,
         };
         const VkCopyBufferInfo2 copy = {
            .sType = VK_STRUCTURE_TYPE_COPY_BUFFER_INFO_2,
            .srcBuffer = panvk_buffer_to_handle(&alias),
